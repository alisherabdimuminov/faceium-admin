...
const editUser = async () => {
    ...
    form.append("working_time", employee.value.working_time.id);
    form.append("branch", employee.value.branch.id);
    ...
    try {
        let response = await $fetch<IResponse>(api(`users/${employee.value.uuid}/edit`), {
            method: "post",
            body: form,
        });
...
const getUser = async () => {
    const response = await $fetch<IResponse>(api(`users/${route.params.uuid}`), {
        method: "get",
    });
...
<template>
    <div class="flex flex-col gap-5 p-5" v-if="employee">
        <div class="flex flex-col md:flex-row gap-5">
            <Card class="w-full bg-accent/30">
                ...
                <CardContent>
                    ...
                    <div>
                        ...
                        <Select v-model="employee.working_time.id">
                            <SelectTrigger>
            ...
            <Card class="w-full md:w-80 h-80 bg-accent/30">
                ...
                <CardContent class="flex flex-col items-center gap-2">
                    <img class="w-40 h-44 rounded-md object-cover" :src="api(employee.image, 'base')" alt="">
        ...
        <Card class="bg-accent/30">
            ...
            <CardContent>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <div>
                        <Label>Filial</Label>
                        <Select v-model="employee.branch.id">
                            <SelectTrigger>
